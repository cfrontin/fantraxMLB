...
def load_war_room_data(percentile=50):
  ...
  df_pecota_all = merge_hittingpitching(df_pecota_hitting, df_pecota_pitching, has_fantrax=True)
...
def load_draft(df_merge=None, debug=False):
  ...
  df_teams.rename(columns={"id": "teamId", "name": "teamName", "division": "teamDiv"}, inplace=True)

  df_draft_in = pd.DataFrame(draft["draftPicks"]).sort_values(["round", "pick"]).reset_index(drop=True)
  if debug:
  ...
  if df_merge is not None:
    df_draft_out = df_draft_out.merge(df_merge, left_on="playerId", right_on="fantraxid")
    df_merge_out = df_merge_out.merge(df_draft_in, left_on="fantraxid", right_on="playerId", how="left")
